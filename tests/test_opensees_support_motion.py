...
from qrest_model.backends.opensees_support_motion import (
    run_euler_imposed_support_result,
    run_shear_imposed_support_result,
)
...
def _undamped_two_story_euler_raw() -> dict:
    return {
        "schema_version": "2.0",
        "model": {
            "type": "euler_beam_2d",
        ...
        "sensors": [
            {"id": "01f_u", "story": 1, "dof": "U", "quantity": "accel"},
            {"id": "02f_theta", "story": 2, "dof": "Theta", "quantity": "disp", "kind": "virtual"},
        ],
        "damping": {"type": "rayleigh", "zeta": 0.0, "modes": [1, 2]},
        "ground_motion": {
            "dt": 0.005,
...
@pytest.mark.opensees
def test_opensees_euler_imposed_support_motion_matches_direct_equivalent_excitation() -> None:
    _require_opensees_tests()
    raw = _undamped_two_story_euler_raw()

    direct = run_analysis(raw, backend="direct")
    imposed = run_euler_imposed_support_result(raw)

    assert imposed.metadata.backend == "opensees_euler_imposed_support"
    assert imposed.metadata.extras["base_excitation_source"] == "OpenSees MultipleSupport imposedMotion"
    assert imposed.metadata.extras["response_source"] == "opensees_imposed_support_motion"
    assert np.allclose(direct.mass_matrix, imposed.mass_matrix)
    assert np.allclose(direct.stiffness_matrix, imposed.stiffness_matrix)
