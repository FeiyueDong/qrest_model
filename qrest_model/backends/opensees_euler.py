"""OpenSeesPy backend for two-dimensional Euler-Bernoulli beam models."""
...
def run_result(config: EulerBeamModelConfig | str | Path) -> AnalysisResult:
    ...
    return AnalysisResult(
        ...
        metadata=AnalysisMetadata(
            backend="opensees_euler",
            response_definition=(
                "OpenSees elasticBeamColumn response for Euler-Bernoulli [u, theta] DOFs; "
...
def _run_opensees(
    ...
    for step in range(n_steps):
        if step > 0:
            ok = ops.analyze(1, dt)
            if ok != 0:
                raise RuntimeError(f"OpenSees Euler analysis failed at step {step} with code {ok}.")
        for story_index, tag in enumerate(node_tags):
...
def _modal_frequency_hz(ops: Any, mode_count: int) -> np.ndarray:
    ...
    if np.any(eigenvalues <= 0.0):
        raise ValueError("OpenSees Euler eigenvalues must be positive.")
