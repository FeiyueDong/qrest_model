"""OpenSeesPy backend for two-dimensional Rayleigh beam models."""
...
from qrest_model.theory.rayleigh_beam import base_excitation_influence, base_excitation_load_vector
...
def run_result(config: RayleighBeamModelConfig | str | Path) -> AnalysisResult:
    ...
    return AnalysisResult(
        ...
        metadata=AnalysisMetadata(
            backend="opensees_rayleigh",
            response_definition=(
                "OpenSees elasticBeamColumn plus nodal rotational mass for Rayleigh [u, theta] DOFs; "
...
def _run_opensees(
    ops: Any,
    config: RayleighBeamModelConfig,
    ...
    for step in range(n_steps):
        if step > 0:
            ok = ops.analyze(1, dt)
            if ok != 0:
                raise RuntimeError(f"OpenSees Rayleigh analysis failed at step {step} with code {ok}.")
        for story_index, tag in enumerate(node_tags):
...
def _modal_frequency_hz(ops: Any, mode_count: int) -> np.ndarray:
    ...
    if np.any(eigenvalues <= 0.0):
        raise ValueError("OpenSees Rayleigh eigenvalues must be positive.")
